...
#line 1 "tests/test_evil.toml.lfs.tc"
#line 1 "lfs.c"
/*
...
void test_case1(
        __attribute__((unused)) intmax_t INVALSET,
        __attribute__((unused)) intmax_t TAIL_TYPE) {
    // prologue
    ...
    // test case 1
    #line 11 "tests/test_evil.toml"
    // create littlefs
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_format(&lfs, &cfg), 0);

    // change tail-pointer to invalid pointers
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_init(&lfs, &cfg), 0);
    lfs_mdir_t mdir;
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_fetch(&lfs, &mdir, (lfs_block_t[2]){0, 1}), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_commit(&lfs, &mdir, LFS_MKATTRS(
            {LFS_MKTAG(LFS_TYPE_HARDTAIL, 0x3ff, 8),
                (lfs_block_t[2]){
                    (INVALSET & 0x1) ? 0xcccccccc : 0,
                    (INVALSET & 0x2) ? 0xcccccccc : 0}})), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_deinit(&lfs), 0);
...
void test_case2(
    ...
    // test case 2
    #line 33 "tests/test_evil.toml"
    // create littlefs
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_format(&lfs, &cfg), 0);
    // make a dir
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mount(&lfs, &cfg), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mkdir(&lfs, "dir_here"), 0);
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_get(&lfs, &mdir,
            LFS_MKTAG(0x700, 0x3ff, 0),
            LFS_MKTAG(LFS_TYPE_NAME, 1, strlen("dir_here")), buffer)
, LFS_MKTAG(LFS_TYPE_DIR, 1, strlen("dir_here")));
    __LFS_ASSERT_MEM_EQ(__FILE__, __LINE__, (char*)buffer, "dir_here", strlen("dir_here"));
    // change dir pointer
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_commit(&lfs, &mdir, LFS_MKATTRS(
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mount(&lfs, &cfg), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_stat(&lfs, "dir_here", &info), 0);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, info.name, "dir_here");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.type, LFS_TYPE_DIR);

    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_open(&lfs, &dir, "dir_here"), LFS_ERR_CORRUPT);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_stat(&lfs, "dir_here/file_here", &info), LFS_ERR_CORRUPT);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_open(&lfs, &dir, "dir_here/dir_here"), LFS_ERR_CORRUPT);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "dir_here/file_here",
...
void test_case3(
    ...
    // test case 3
    #line 79 "tests/test_evil.toml"
    // create littlefs
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_format(&lfs, &cfg), 0);
    // make a file
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mount(&lfs, &cfg), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "file_here",
...
, LFS_MKTAG(LFS_TYPE_REG, 1, strlen("file_here")));
    __LFS_ASSERT_MEM_EQ(__FILE__, __LINE__, (char*)buffer, "file_here", strlen("file_here"));
    // change file pointer
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_commit(&lfs, &mdir, LFS_MKATTRS(
    ...
    // of ways to access the dir, some can fail, but some don't
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mount(&lfs, &cfg), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_stat(&lfs, "file_here", &info), 0);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, info.name, "file_here");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.type, LFS_TYPE_REG);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.size, SIZE);

    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "file_here", LFS_O_RDONLY), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_read(&lfs, &file, buffer, SIZE), LFS_ERR_CORRUPT);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);

    // any allocs that traverse CTZ must unfortunately must fail
    if (SIZE > 2*LFS_BLOCK_SIZE) {
        __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mkdir(&lfs, "dir_here"), LFS_ERR_CORRUPT);
    }
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_unmount(&lfs), 0);
...
void test_case4(
    ...
    for (int i = 0; i < SIZE; i++) {
        char c = 'c';
        __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_write(&lfs, &file, &c, 1), 1);
    }
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_unmount(&lfs), 0);
...
, LFS_MKTAG(LFS_TYPE_CTZSTRUCT, 1, sizeof(struct lfs_ctz)));
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, cfg.read(&cfg, ctz.head, 0, bbuffer, LFS_BLOCK_SIZE), 0);
    uint32_t bad = lfs_tole32(0xcccccccc);
    memcpy(&bbuffer[0], &bad, sizeof(bad));
    memcpy(&bbuffer[4], &bad, sizeof(bad));
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, cfg.erase(&cfg, ctz.head), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, cfg.prog(&cfg, ctz.head, 0, bbuffer, LFS_BLOCK_SIZE), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_deinit(&lfs), 0);
...
void test_case5(
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_commit(&lfs, &mdir, NULL, 0), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_deinit(&lfs), 0);
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mount(&lfs, &cfg), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mkdir(&lfs, "should_fail"), LFS_ERR_CORRUPT);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_unmount(&lfs), 0);
...
, LFS_MKTAG(LFS_TYPE_DIRSTRUCT, 1, sizeof(pair)));
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_fetch(&lfs, &mdir, pair), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_commit(&lfs, &mdir, LFS_MKATTRS(
            {LFS_MKTAG(LFS_TYPE_HARDTAIL, 0x3ff, 8),
                (lfs_block_t[2]){0, 1}})), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_deinit(&lfs), 0);
