...
#line 1 "tests/test_move.toml.lfs.tc"
#line 1 "lfs.c"
/*
...
static int lfs_bd_read(lfs_t *lfs,
    ...
    while (size > 0) {
        ...
        __LFS_ASSERT_INT_LT(__FILE__, __LINE__, block, lfs->cfg->block_count);
        rcache->block = block;
        ...
        __LFS_ASSERT_INT_LE(__FILE__, __LINE__, err, 0);
        if (err) {
            return err;
        }
    }
...
static int lfs_bd_flush(lfs_t *lfs,
    ...
    if (pcache->block != LFS_BLOCK_NULL && pcache->block != LFS_BLOCK_INLINE) {
        __LFS_ASSERT_INT_LT(__FILE__, __LINE__, pcache->block, lfs->cfg->block_count);
...
static int lfs_bd_prog(lfs_t *lfs,
        ...
        const void *buffer, lfs_size_t size) {
    const uint8_t *data = buffer;
    __LFS_ASSERT_BOOL_EQ(__FILE__, __LINE__, block == LFS_BLOCK_INLINE || block < lfs->cfg->block_count, true);
    __LFS_ASSERT_INT_LE(__FILE__, __LINE__, off + size, lfs->cfg->block_size);
    ...
    while (size > 0) {
        ...
        __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, pcache->block, LFS_BLOCK_NULL);
    ...
    while (true) {
        {
            ...
            __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, commit.off % lfs->cfg->prog_size, 0);
            lfs_pair_swap(dir->pair);
            dir->count = end - begin;
            dir->off = commit.off;
...
static int lfs_dir_commit(lfs_t *lfs, lfs_mdir_t *dir,
    ...
    for (int i = 0; i < attrcount; i++) {
        if (lfs_tag_type3(attrs[i].tag) == LFS_TYPE_CREATE) {
            ...
            __LFS_ASSERT_INT_GT(__FILE__, __LINE__, dir->count, 0);
            dir->count -= 1;
            hasdelete = true;
...
#else
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, (flags & LFS_O_RDONLY), LFS_O_RDONLY);
#endif
...
static lfs_ssize_t lfs_file_rawread(lfs_t *lfs, lfs_file_t *file,
        void *buffer, lfs_size_t size) {
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, (file->flags & LFS_O_RDONLY), LFS_O_RDONLY);
...
static lfs_ssize_t lfs_file_rawwrite(lfs_t *lfs, lfs_file_t *file,
        const void *buffer, lfs_size_t size) {
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, (file->flags & LFS_O_WRONLY), LFS_O_WRONLY);
...
static int lfs_init(lfs_t *lfs, const struct lfs_config *cfg) {
    ...
    __LFS_ASSERT_INT_NE(__FILE__, __LINE__, lfs->cfg->read_size, 0);
    __LFS_ASSERT_INT_NE(__FILE__, __LINE__, lfs->cfg->prog_size, 0);
    __LFS_ASSERT_INT_NE(__FILE__, __LINE__, lfs->cfg->cache_size, 0);

    // check that block size is a multiple of cache size is a multiple
    // of prog and read sizes
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs->cfg->cache_size % lfs->cfg->read_size, 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs->cfg->cache_size % lfs->cfg->prog_size, 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs->cfg->block_size % lfs->cfg->cache_size, 0);

    // check that the block size is large enough to fit ctz pointers
    __LFS_ASSERT_INT_LE(__FILE__, __LINE__, 4*lfs_npw2(0xffffffff / (lfs->cfg->block_size-2*4))
, lfs->cfg->block_size);
    ...
    // wear-leveling.
    __LFS_ASSERT_INT_NE(__FILE__, __LINE__, lfs->cfg->block_cycles, 0);
    ...
    __LFS_ASSERT_INT_GT(__FILE__, __LINE__, lfs->cfg->lookahead_size, 0);
    __LFS_ASSERT_BOOL_EQ(__FILE__, __LINE__, lfs->cfg->lookahead_size % 8 == 0 &&
            (uintptr_t)lfs->cfg->lookahead_buffer % 4 == 0, true);
    ...
    __LFS_ASSERT_INT_LE(__FILE__, __LINE__, lfs->cfg->name_max, LFS_NAME_MAX);
    lfs->name_max = lfs->cfg->name_max;
    if (!lfs->name_max) {
        lfs->name_max = LFS_NAME_MAX;
    }

    __LFS_ASSERT_INT_LE(__FILE__, __LINE__, lfs->cfg->file_max, LFS_FILE_MAX);
    lfs->file_max = lfs->cfg->file_max;
    if (!lfs->file_max) {
        lfs->file_max = LFS_FILE_MAX;
    }

    __LFS_ASSERT_INT_LE(__FILE__, __LINE__, lfs->cfg->attr_max, LFS_ATTR_MAX);
    lfs->attr_max = lfs->cfg->attr_max;
    if (!lfs->attr_max) {
        lfs->attr_max = LFS_ATTR_MAX;
    }

    __LFS_ASSERT_INT_LE(__FILE__, __LINE__, lfs->cfg->metadata_max, lfs->cfg->block_size);
...
#ifndef LFS_READONLY
static int lfs_fs_preporphans(lfs_t *lfs, int8_t orphans) {
    __LFS_ASSERT_BOOL_EQ(__FILE__, __LINE__, lfs_tag_size(lfs->gstate.tag) > 0 || orphans >= 0, true);
...
int lfs_file_open(lfs_t *lfs, lfs_file_t *file, const char *path, int flags) {
    ...
    __LFS_ASSERT_BOOL_EQ(__FILE__, __LINE__, !lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)file), true);
...
int lfs_dir_open(lfs_t *lfs, lfs_dir_t *dir, const char *path) {
    ...
    __LFS_ASSERT_BOOL_EQ(__FILE__, __LINE__, !lfs_mlist_isopen(lfs->mlist, (struct lfs_mlist*)dir), true);
...
void test_case3() {
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, cfg.read(&cfg, block, 0, bbuffer, LFS_BLOCK_SIZE), 0);
    int off = LFS_BLOCK_SIZE-1;
    while (off >= 0 && bbuffer[off] == LFS_ERASE_VALUE) {
        off -= 1;
    }
    memset(&bbuffer[off-3], LFS_BLOCK_SIZE, 3);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, cfg.erase(&cfg, block), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, cfg.prog(&cfg, block, 0, bbuffer, LFS_BLOCK_SIZE), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, cfg.sync(&cfg), 0);
...
void test_case4() {
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "a/hello", LFS_O_RDONLY), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_read(&lfs, &file, buffer, 5), 5);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, memcmp(buffer, "hola\n", 5), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_read(&lfs, &file, buffer, 8), 8);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, memcmp(buffer, "bonjour\n", 8), 0);
...
void test_case9() {
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_open(&lfs, &dir, "a/hi"), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 1);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, info.name, ".");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.type, LFS_TYPE_DIR);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 1);
...
void test_case16(
        __attribute__((unused)) intmax_t RELOCATIONS) {
    // prologue
    __attribute__((unused)) lfs_t lfs;
    __attribute__((unused)) lfs_testbd_t bd;
    __attribute__((unused)) lfs_file_t file;
    ...
    // test case 16
    #line 1526 "tests/test_move.toml"
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_format(&lfs, &cfg), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mount(&lfs, &cfg), 0);

    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mkdir(&lfs, "/parent"), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mkdir(&lfs, "/parent/child"), 0);

    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/1.move_me",
            LFS_O_WRONLY | LFS_O_CREAT), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_write(&lfs, &file, "move me",
            sizeof("move me")), sizeof("move me"));
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);

    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/0.before",
            LFS_O_WRONLY | LFS_O_CREAT), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_write(&lfs, &file, "test.1", 7), 7);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/2.after",
            LFS_O_WRONLY | LFS_O_CREAT), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_write(&lfs, &file, "test.2", 7), 7);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/child/0.before",
            LFS_O_WRONLY | LFS_O_CREAT), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_write(&lfs, &file, "test.3", 7), 7);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/child/2.after",
            LFS_O_WRONLY | LFS_O_CREAT), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_write(&lfs, &file, "test.4", 7), 7);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);

    lfs_file_t files[4];
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &files[0], "/parent/0.before",
            LFS_O_WRONLY | LFS_O_TRUNC), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &files[1], "/parent/2.after",
            LFS_O_WRONLY | LFS_O_TRUNC), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &files[2], "/parent/child/0.before",
            LFS_O_WRONLY | LFS_O_TRUNC), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &files[3], "/parent/child/2.after",
            LFS_O_WRONLY | LFS_O_TRUNC), 0);
    ...
    if (RELOCATIONS & 0x1) {
        ...
        __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_testbd_setwear(&cfg, dir.m.pair[0], 0xffffffff), 0);
        __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_testbd_setwear(&cfg, dir.m.pair[1], 0xffffffff), 0);
        __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_close(&lfs, &dir), 0);
    }
    if (RELOCATIONS & 0x2) {
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_rename(&lfs, "/parent/1.move_me", "/parent/child/1.move_me"), 0);
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_open(&lfs, &dir, "/parent"), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 1);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, info.name, ".");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.type, LFS_TYPE_DIR);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 1);
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.size, 7);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 1);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, info.name, "child");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.type, LFS_TYPE_DIR);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_close(&lfs, &dir), 0);

    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_open(&lfs, &dir, "/parent/child"), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 1);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, info.name, ".");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.type, LFS_TYPE_DIR);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 1);
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.type, LFS_TYPE_REG);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.size, sizeof("move me"));
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 1);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, info.name, "2.after");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.type, LFS_TYPE_REG);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.size, 7);
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/0.before", LFS_O_RDONLY), 0);
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/2.after", LFS_O_RDONLY), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_read(&lfs, &file, buffer, 7), 7);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, (char*)buffer, "test.6");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/child/0.before", LFS_O_RDONLY), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_read(&lfs, &file, buffer, 7), 7);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, (char*)buffer, "test.7");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/child/2.after", LFS_O_RDONLY), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_read(&lfs, &file, buffer, 7), 7);
...
void test_case17(
    ...
    // test case 17
    #line 1663 "tests/test_move.toml"
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_format(&lfs, &cfg), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mount(&lfs, &cfg), 0);

    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mkdir(&lfs, "/parent"), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mkdir(&lfs, "/parent/child"), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mkdir(&lfs, "/parent/sibling"), 0);

    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/sibling/1.move_me",
            LFS_O_WRONLY | LFS_O_CREAT), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_write(&lfs, &file, "move me",
            sizeof("move me")), sizeof("move me"));
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);

    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/sibling/0.before",
            LFS_O_WRONLY | LFS_O_CREAT), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_write(&lfs, &file, "test.1", 7), 7);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/sibling/2.after",
            LFS_O_WRONLY | LFS_O_CREAT), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_write(&lfs, &file, "test.2", 7), 7);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/child/0.before",
    ...
    lfs_file_t files[4];
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &files[0], "/parent/sibling/0.before",
            LFS_O_WRONLY | LFS_O_TRUNC), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &files[1], "/parent/sibling/2.after",
            LFS_O_WRONLY | LFS_O_TRUNC), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &files[2], "/parent/child/0.before",
            LFS_O_WRONLY | LFS_O_TRUNC), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &files[3], "/parent/child/2.after",
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_rename(&lfs,
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.type, LFS_TYPE_DIR);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 1);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, info.name, "sibling");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.type, LFS_TYPE_DIR);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_close(&lfs, &dir), 0);

    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_open(&lfs, &dir, "/parent/sibling"), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 1);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, info.name, ".");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, info.type, LFS_TYPE_DIR);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_dir_read(&lfs, &dir, &info), 1);
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/sibling/0.before", LFS_O_RDONLY), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_read(&lfs, &file, buffer, 7), 7);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, (char*)buffer, "test.5");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/sibling/2.after", LFS_O_RDONLY), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_read(&lfs, &file, buffer, 7), 7);
    __LFS_ASSERT_STR_EQ(__FILE__, __LINE__, (char*)buffer, "test.6");
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "/parent/child/0.before", LFS_O_RDONLY), 0);
