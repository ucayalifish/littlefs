...
#line 1 "tests/test_alloc.toml.lfs.tc"
#line 1 "lfs.c"
/*
...
void test_case8() {
    ...
    // test case 8
    #line 331 "tests/test_alloc.toml"
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "pacman", LFS_O_WRONLY | LFS_O_CREAT), 0);
    strcpy((char*)buffer, "waka");
    size = strlen("waka");
    lfs_size_t filesize = 0;
    while (true) {
        lfs_ssize_t res = lfs_file_write(&lfs, &file, buffer, size);
        __LFS_ASSERT_BOOL_EQ(__FILE__, __LINE__, res == (lfs_ssize_t)size || res == LFS_ERR_NOSPC, true);
    ...
    // scan to bail early
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_testbd_setwear(&cfg, fileblock, 0xffffffff), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "ghost", LFS_O_WRONLY | LFS_O_CREAT), 0);
    strcpy((char*)buffer, "chomp");
    size = strlen("chomp");
    while (true) {
        lfs_ssize_t res = lfs_file_write(&lfs, &file, buffer, size);
        __LFS_ASSERT_BOOL_EQ(__FILE__, __LINE__, res == (lfs_ssize_t)size || res == LFS_ERR_CORRUPT, true);
        if (res == LFS_ERR_CORRUPT) {
            break;
        }
    }
    ...
    // run out of space
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_testbd_setwear(&cfg, fileblock, 0), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "ghost", LFS_O_WRONLY | LFS_O_CREAT), 0);
    strcpy((char*)buffer, "chomp");
    size = strlen("chomp");
    while (true) {
    ...
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_mount(&lfs, &cfg), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_open(&lfs, &file, "pacman", LFS_O_RDONLY), 0);
    strcpy((char*)buffer, "waka");
    size = strlen("waka");
    for (lfs_size_t i = 0; i < filesize/size; i++) {
        uint8_t rbuffer[4];
        __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_read(&lfs, &file, rbuffer, size), size);
        __LFS_ASSERT_MEM_EQ(__FILE__, __LINE__, rbuffer, buffer, size);
    }
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_file_close(&lfs, &file), 0);
    __LFS_ASSERT_INT_EQ(__FILE__, __LINE__, lfs_unmount(&lfs), 0);
